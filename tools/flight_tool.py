...
DEFAULT_ORIGIN_IATA = os.getenv("DEFAULT_ORIGIN_IATA","MVD")
...
def search_flights(origin: str, destination: str, date: str | None = None) -> str:
    """
    Busca vuelos en vivo entre dos ubicaciones usando AviationStack.
 
    Args:
        origin: Ciudad, país o código IATA de origen (ej: "Tokyo", "Japan", "NRT").
        destination: Ciudad, país o código IATA de destino.
        date: Fecha opcional en formato YYYY-MM-DD, solo si el usuario la mencionó.
    """
    ...
    if not dep_iata and not arr_iata:
         return (
            f"Could not resolve '{origin}' or '{destination}' to a known airport. "
            "Try a major city name, country name, or a 3-letter IATA code."
        )
    ...
    if not flight_data:
        route_text =  f" from {dep_iata or '?'} to {arr_iata or '?'}"
        date_text = f" on {date}" if date else ""

        return (
            f"No live flight data found{route_text}{date_text}.\n\n"
    ...
    route_info = f"Live flights from {dep_iata or 'anywhere'} to {arr_iata or 'anywhere'}"

    formatted_flights = [format_flight(flight) for flight in flight_data[:10]]
